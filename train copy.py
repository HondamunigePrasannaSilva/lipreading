...
def create(config):
    ...
    # define the models
    model = model_experiment.only_Decoder2(config.INPUT_DIM, config.HID_DIM, config.NUM_LAYERS, len(vocabulary)).to(device)
...
def train(model, ctc_loss, optimizer,trainloader, vocabulary, config,valloader, modeltitle= "_new"):
