...
"""-------TRAIN OF LSTM MODELS-------"""
"""OnlyDecoder, Seq2Seq, OnlyDecoder_lin_emb, OnlyDecoder_MLP_emb
NB: To train OnlyDecoder_MLP_emb you need to specify a path to a pretrained MLP_emb network.
    Either with the Transformer pipeline or the train_embedding Pipeline
"""
...
def create(config):
    ...
    #IF YOU WANT TO TRAIN oNLYDECODER with linear embeddings
    #model = only_Decoder_linear_emb(config.INPUT_DIM, config.HID_DIM, config.NUM_LAYERS, len(vocabulary)).to(device)

    #IF YOU WANT TO TRAIN oNLYDECODER with pretrained MLP embeddings.
    model = only_Decoder_MLP_emb(config.INPUT_DIM, config.HID_DIM, config.NUM_LAYERS, len(vocabulary), path_emb = "models/model_embexp6_trvalloss070.pt").to(device)
