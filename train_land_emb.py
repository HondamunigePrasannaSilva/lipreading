...
def train(model, ctc_loss, optimizer,trainloader, vocabulary, config,valloader, modeltitle= "_AV"):
    ...
    model_emb = MLP_emb().to(device)
    model_emb.load_state_dict(torch.load("./models/model_embedding_0.pt"))
    model_emb.eval()
    # Training loop
    ...
    for epoch in range(config.EPOCHS):
        ...
        for landmarks, len_landmark, label, len_label in trainloader:
            ...
            with torch.no_grad():
                land_emb = model_emb(landmarks)

            output, _, _ = model(land_emb, len_landmark)
...
def test(model, valloader, vocabulary, ctc_loss):
    ...
    model_emb = MLP_emb().to(device)
    model_emb.load_state_dict(torch.load("./models/model_embedding.pt"))
    model_emb.eval()
